...
phash_schema_drift = Counter(
    "ocr_phash_schema_drift_total", "Cached JSON failed InvoiceResult.model_validate"
)

# ---- YOLO / Triton ----
yolo_rejection_total = Counter(
    "ocr_yolo_rejection_total", "YOLO produced no detection above threshold"
)
triton_batch_size = Histogram(
    "ocr_triton_batch_size", "Triton-reported batch size per response"
)

# ---- Gemini ----
gemini_retries_total = Counter(
    "ocr_gemini_retries_total", "Gemini 5xx/timeout retry attempts", ["attempt"]
)
...
# ---- rate limit / yield ----
rate_limit_yields_total = Counter(
    "ocr_rate_limit_yields_total", "Worker yielded to queue due to rate limit"
)
requeue_count = Histogram(
    "ocr_requeue_count", "Observed requeue count per yield event"
)
token_bucket_acquire_total = Counter(
    "ocr_token_bucket_acquire_total", "Token bucket try_acquire outcomes", ["outcome"]
)
token_bucket_refresh_total = Counter(
    "ocr_token_bucket_refresh_total", "Rate-limit refresh tick outcomes", ["outcome"]
)
token_bucket_available = Gauge(
    "ocr_token_bucket_available", "Current tokens available (sampled at refresh tick)"
)
