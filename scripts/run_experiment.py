...
def _run_eval(input_path: str, psv: str, split: str, workers: int, extra_args: list[str]) -> Path:
...
def cmd_run(args: argparse.Namespace) -> int:
    if _is_locked():
        print(
            "ERROR: experiments are locked (final eval already run). Remove experiments/locked to reset.",
            file=sys.stderr,
        )
        return 1
    ...
    if run_count >= MAX_RUNS:
        print(
            f"ERROR: budget exhausted ({run_count}/{MAX_RUNS} runs used). "
    ...
    if not Path(DEV_INPUT).exists():
        print(f"ERROR: dev set not found: {DEV_INPUT}\nRun: python scripts/split_eval_set.py", file=sys.stderr)
        return 1
...
def cmd_final(args: argparse.Namespace) -> int:
    if _is_locked():
        print("ERROR: final eval already run. See experiments/final_result.json", file=sys.stderr)
        return 1
    ...
    # Lock
    LOCKED_SENTINEL.write_text(f"Locked at {datetime.now().isoformat()} by final eval psv={args.psv}\n")
...
def main() -> int:
    ...
    # --run
    p_run = sub.add_parser("--run", aliases=["run"], help="Run one dev-set experiment (max 3)")
    ...
    p_run.add_argument("eval_args", nargs="*", help="Extra args forwarded verbatim to run_eval.py")

    # --final
    p_fin = sub.add_parser("--final", aliases=["final"], help="Run final evaluation on test set (once only)")
